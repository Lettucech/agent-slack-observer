...
test("registers every active user-visible conversation over paginated Slack results", async () => {
  ...
  globalThis.fetch = async (url, init) => {
    ...
    if (String(url).includes("cursor=next-page")) return new Response(JSON.stringify({ ok: true, channels: [{ id: "D1" }, { id: "C-archived", name: "old", is_archived: true }] }));
    return new Response(JSON.stringify({ ok: true, channels: [{ id: "C1", name: "general" }, { id: "G1", name: "private" }], response_metadata: { next_cursor: "next-page" } }));
  };
  try {
    ...
    assert.deepEqual(await discovery.discover(), { workspaceId: "T1", conversations: 3 });
    assert.deepEqual(calls.map((call) => call.authorization), ["Bearer xoxp-test", "Bearer xoxp-test", "Bearer xoxp-test"]);
    ...
    assert.deepEqual(registered, [{ workspaceId: "T1", workspaceName: "Engineering", conversations: [
      { channelId: "C1", channelName: "general" }, { channelId: "G1", channelName: "private" }, { channelId: "D1", channelName: null },
    ] }]);
