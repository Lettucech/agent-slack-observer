...
function formatConversation(channel) { return `<div class="conversation-reference"><strong class="reference-name">${escapeHtml(conversationTitle(channel))}</strong><span class="conversation-badge type-${escapeHtml(channel.conversationType ?? "unknown")}">${escapeHtml(conversationTypeLabel(channel.conversationType))}</span><code>${escapeHtml(channel.channelId)}</code></div>`; }
