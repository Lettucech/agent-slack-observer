...
function renderOverview(status, socket, channels, jobs) {
  ...
  $("#overview-channels").innerHTML = coveredChannels.length ? coveredChannels.slice(0, 3).map((channel) => compactRow(channel.channelName ?? channel.channelId, `${channel.messageCount} messages · ${shortTime(channel.lastObservedAt)}`)).join("") : "<p>No conversations are in coverage yet.</p>";
...
function renderDashboardData(status, channels, jobs) {
  ...
  $("#channels").innerHTML = channels.length ? channels.map((channel) => `<tr><td>${formatReference(channel.workspaceName, channel.workspaceId)}</td><td>${formatReference(channel.channelName, channel.channelId)}</td><td>${channel.enabled ? "Included" : "Not included"}</td><td>${channel.messageCount}</td><td>${time(channel.lastObservedAt)}</td><td><button class="secondary tiny" data-set-coverage="${channel.enabled ? "false" : "true"}" data-workspace-id="${escapeHtml(channel.workspaceId)}" data-channel-id="${escapeHtml(channel.channelId)}" type="button">${channel.enabled ? "Exclude" : "Include"}</button></td></tr>`).join("") : "<tr><td colspan=\"6\" class=\"form-status\">Find accessible conversations or include one by ID to begin.</td></tr>";
...
$("#sync-names").addEventListener("click", async () => { const button = $("#sync-names"); setPending(button, true, "Refreshing names"); $("#coverage-status").textContent = "Refreshing names for included conversations…"; try { const { queued } = await post("/dashboard/metadata/sync"); $("#coverage-status").textContent = queued ? `Name lookup queued for ${queued} included conversation${queued === 1 ? "" : "s"}.` : "No included conversations need a name lookup."; await refreshDashboard(); } catch (error) { $("#coverage-status").textContent = "Name refresh could not be queued."; console.error(error); } finally { setPending(button, false); } });
$("#discover-conversations").addEventListener("click", async () => { const button = $("#discover-conversations"); setPending(button, true, "Finding conversations"); $("#discovery-status").textContent = "Reading conversation names and IDs…"; try { const { conversations } = await post("/dashboard/conversations/discover"); $("#discovery-status").textContent = `Found ${conversations} accessible conversation${conversations === 1 ? "" : "s"}. Choose Include in recovery coverage for the ones you want.`; await refreshDashboard(); } catch (error) { $("#discovery-status").textContent = "Accessible conversations could not be found."; console.error(error); } finally { setPending(button, false); } });
