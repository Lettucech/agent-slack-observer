...
2. Under **OAuth & Permissions**, add scopes to the selected **user** or **bot** token only for the conversation types you intend to observe. The dashboard lists the complete scope checklist:
   ...
   - DM and group-DM participant labels (optional): `users:read` (the observer never requests email addresses)
...
7. Test and save the dashboard settings. With a user token, use **Find accessible conversations** to list public channels, private channels, DMs, and group DMs that token can see, then click **Include** only for conversations you want in recovery coverage. With `users:read`, it also resolves Slack display names for DM and group-DM participants. Finding conversations does not read message content or add coverage automatically. Without it, include a quiet channel by workspace and channel ID. Socket Mode channels are included automatically after their first event.

The observer uses the selected read token for `auth.test`, `team.info`, `conversations.info`, and dashboard-created backfill. A user-token selection additionally enables explicit `conversations.list` discovery. The explicit **Find accessible conversations** and **Refresh included names** actions may additionally call `conversations.members` and `users.info` to resolve participant display names; they never request email addresses or message content.
...
| `ack_digest` | Mark one completed delivery receipt with its `ackToken`; it may also record agent-reported `{ inputTokens, outputTokens, durationMs }`. It never changes Slack or deletes retained observer messages. |
...
| `list_channels` | List observed conversations, including their IDs, cached names, and `conversationType`. |
| `get_observer_status` | Read local counts and latest-received time. |

`get_digest_batches` takes `maxBytes`, a provider-neutral UTF-8 digest-content allowance selected by the calling agent from its remaining context. The observer does not assume an OpenAI or other provider tokenizer. Compact digest results include timestamps, non-empty author/subtype fields, conversation labels, and text, but never raw Slack payloads, event IDs, or diagnostic estimates. The opaque acknowledgement receipt is additional bounded metadata. If one message's text cannot fit, the result contains a lossless first segment plus `textContinues`, a Unicode code-point offset for `get_message_digest`; no text is discarded. New messages are held for the configured thread-settle window (90 seconds by default) after the latest activity, reducing the chance of digesting a thread while it is still active.

For the consumer inbox workflow, an agent chooses a stable `consumerId`, calls `get_digest_batches`, then calls `ack_digest` with the opaque `ackToken` returned with each complete group. When its runtime exposes measured usage for that digest operation, it should also send `usage: { inputTokens, outputTokens, durationMs }`; the local dashboard stores that as agent-reported usage rather than attempting to infer provider tokens. A receipt retry cannot duplicate that record. A long thread that has already been acknowledged and later reopens may return `checkpointSuggested`; the agent may attach a bounded, source-linked checkpoint while acknowledging it. Later reads receive this consumer-private checkpoint plus new replies, and can always request retained raw thread text when ambiguity needs verification. Checkpoints are local PostgreSQL data, are scoped to one consumer, and are removed when their retained root message expires. Acknowledgements remain idempotent and never alter Slack or delete retained observer messages.
